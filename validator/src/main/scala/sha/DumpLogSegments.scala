...
object DumpLogSegments {
  val kafkaValidate = new KafkaValidate()
    ...
    override def parse(message: Message): (Option[K], Option[V]) = {
      if (message.isNull) {
        ...
        val key = if (message.hasKey)
          Some(keyDecoder.fromBytes(utils.Utils.readBytes(message.key)))
        else
          None

        val payload = Some(valueDecoder.fromBytes(utils.Utils.readBytes(message.payload)))
    ...
    private def parseGroupMetadata(groupMetadataKey: GroupMetadataKey, payload: ByteBuffer) = {
      ...
      val assignment = group.allMemberMetadata.map { member =>
        if (protocolType == ConsumerProtocol.PROTOCOL_TYPE) {
          ...
          val userData = hex(utils.Utils.toArray(partitionAssignment.userData()))
  ...
  private def dumpLog(file: File,
    ...
    var messageSet:FileMessageSet = null
    try {
...
//          payload.map(payload => print(s" payload: ${payload}"))
          payload.map(payload => kafkaValidate.go(fileName, payload))
          count += 1
          if(count >= 10000) {
            kafkaValidate.finish()
            return
